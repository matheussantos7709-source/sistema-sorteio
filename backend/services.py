...
def _norm_key_piece(s: str) -> str:
    """
    Normalização forte pra gerar chave estável:
    - lower
    - sem acento
    - remove pontuação
    - colapsa espaços
...
def _is_bloqueado_permanente(chave: str) -> bool:
    ...
    try:
        cur = conn.cursor()
        cur.execute("SELECT 1 FROM bloqueados_permanentes WHERE chave=%s LIMIT 1", (chave,))
...
# -----------------------------
# CADASTRAR / ATUALIZAR (UPSERT por CHAVE)
# -----------------------------
...
def cadastrar_participante(nome, email, cpf, whatsapp, curso, perfil, semestre):
    ...
    email = _norm_spaces(email)
    cpf = _norm_spaces(cpf)
    whatsapp = _norm_spaces(whatsapp)
    curso = _norm_spaces(curso)
    perfil = _norm_spaces(perfil)
    semestre = _norm_spaces(semestre)
...
def exportar_participantes_xlsx() -> bytes:
    ...
    cur.execute(
        """
        SELECT id, chave, nome, email, cpf, whatsapp, curso, perfil, semestre,
...
# -----------------------------
# IMPORTAR XLSX (ignora bloqueados permanentes)
# -----------------------------
...
def importar_participantes_xlsx(file_bytes: bytes) -> Dict[str, Any]:
    ...
    return {
        "ok": True,
        "msg": f"Importação finalizada. {importados} linha(s) importada(s). {ignorados} ignorada(s) por bloqueio permanente.",
